...
class Yolov3:
    def __init__(self, classesfile, configfile, weightfile):
        ...
        # self.net = cv2.dnn.readNetFromONNX(weightfile) #YOLOv5 need to shift .pt to .onnx
        self.C = np.array([])

    def get_output_layers(self, net):
        layer_names = net.getLayerNames()
        # output_layers = [layer_names[i[0] - 1] for i in net.getUnconnectedOutLayers()]
        # print(layer_names)#('conv_0', 'bn_0', 'leaky_1', 'conv_1', 'bn_1', 'leaky_2', ...)
    ...
    def detect(self, image, Width, Height):
        ...
        if len(confidences) > 0:
            self.C = np.append(self.C, confidences)
