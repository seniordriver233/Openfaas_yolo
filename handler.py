...
def handle(username):
    ...
    vidcap = cv2.VideoCapture(username)  # video from: https://www.youtube.com/watch?v=vF1RPI6j7b0
    # vidcap = cv2.VideoCapture(temp.getVideoTemppath(username))
    fps = vidcap.get(cv2.CAP_PROP_FPS)
    print("Frames per second using video.get(cv2.CAP_PROP_FPS) : {0}".format(fps))
    success, image = vidcap.read()
    # img_bytes = cv2.imencode('.jpg', image)[1].tobytes()
    # temp.r.saveVideo(username,img_bytes)
    # image = temp.r.readVideo(username)
    ...
    print("len:",len(yolo.C))
    print("The mean is: ", np.mean(yolo.C))
    print("STD:", np.std(yolo.C))
